...
RSpec.describe ResourceEstimator do
  ...
  it 'returns nil if there is a raid report with no acropolis' do
    raid = double('Raid', reported_at: Time.now)
  ...
  it 'returns the value of the last scout report with no raid reports' do
    scout = double('Scout', reported_at: Time.now, timber: 3000, bronze: 4000, food: 5000)
  ...
  it 'returns nil if there is a raid after the last scout without an acropolis value' do
    raid = double('Raid', reported_at: Time.now, at_capacity?: false)
    scout = double('Scout', reported_at: Time.now - 1.hour, timber: 1, bronze: 2, food: 3)
  ...
  it 'returns the adjusted value from the last scout report when production values are set' do
    scout = double('Scout', reported_at: Time.now - 90.minutes, timber: 1000, bronze: 2000, food: 3000)
  ...
  it 'returns the adjusted value from the last raid report with production and acropolis set' do
    raid = double('Raid', reported_at: Time.now - 90.minutes, at_capacity?: false)
  ...
  it 'returns nil if the last raid hit capacity and no previoius scout report' do
    raid = double('Raid', reported_at: Time.now, at_capacity?: true, timber: 9500, bronze: 9500, food: 9500)
    city = double('City', acropolis: nil, raids: [raid], scouts: [], timber_production: nil, bronze_production: nil, food_production: nil)
  ...
  it 'returns the difference between the previous scout and current raid report when at capacity' do
    scout = double('Scout', reported_at: Time.now - 2.hour, timber: 15000, bronze: 15000, food: 15000)
    raid = double('Raid', reported_at: Time.now - 1.hour, at_capacity?: true, timber: 9500, bronze: 9500, food: 9500)
  ...
  it 'returns the storage cap if the production overflows it' do
    scout = double('Scout', reported_at: Time.now - 12.hours, timber: 1000, bronze: 1000, food: 1000)
